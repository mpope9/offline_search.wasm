/*
 * Implementationn of Javascript to interact with the offline_search WebAssembly.
 */
export class OfflineSearch {
  ...
  async init() {
    ...
    await this.Module.then(function(module) {
      indexSize = module._initialize_index();
    });
  ...
  async search(input) {
    ...
    var output = await this.Module.then(function(module) {
      ...
      var initOutputPtr = module._malloc(idxSize * module.HEAP8.BYTES_PER_ELEMENT);
